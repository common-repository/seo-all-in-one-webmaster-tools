 ...
 * Recipe
 * 2015
 * @copyright Nick Gejadze
 * @version 1.0
 * @website http://codecanyon.net/user/Gejadze
 */
...
class Recipe
{
    ...
    /**
     * Get Twitter Share count for a URL
     * @param  string $url The URL to get the Twitter share count for
     * @return int  number of shares
     */
    public static function getTwitterShareCount($url) {
        $providerURL = "https://cdn.api.twitter.com/1/urls/count.json?url=";
        $json = self::curl($providerURL . $url);
     ...
     * @param  string $url The URL to get the Facebook share count for
     * @return int  number of shares
     */
    public static function getFacebookShareCount($url) {
        $providerURL = "http://graph.facebook.com/?id=";
        if (strpos($url, "http") === false):
            $url = 'http://' . $url;
        endif;
        $json = self::curl($providerURL . $url);
    ...
    /**
     * Get Facebook Likes count for a url
     * @param  string $page The URL to get the Facebook likes count for
     * @return int  number of likes
     */
    public static function getFacebookLikesCount($page) {
        $providerURL = "http://graph.facebook.com/";
        if (strpos($page, "http") !== false):
            $page = preg_replace('~^https?://~', '', $page);
        endif;
        $json = self::curl($providerURL . $page);
        $data = json_decode($json, true);
        if (isset($data['likes'])):
    ...
    /**
     * Get Google Plus share count for a url
     * @param  string $url The URL to get the Google Plus share count for
     * @return int  number of likes
     */
    public static function getGooglePlusShareCount($url) {
        if (strpos($url, "http") === false):
            $url = 'http://' . $url;
        endif;
        $data = self::curl("https://clients6.google.com/rpc?key=AIzaSyCKSbrvQasunBoV16zDH9R33D88CeLr9gQ", "POST", '[{"method":"pos.plusones.get","id":"p","params":{"nolog":true,"id":"' . $url . '","source":"widget","userId":"@viewer","groupId":"@self"},"jsonrpc":"2.0","key":"p","apiVersion":"v1"}]', array('Content-type: application/json'));
        $parsed_results = json_decode($data, true);
        if (isset($parsed_results[0]['result']['metadata']['globalCounts']['count']) and is_numeric($parsed_results[0]['result']['metadata']['globalCounts']['count'])):
            return (int)$parsed_results[0]['result']['metadata']['globalCounts']['count'];
        endif;
        return false;
    }
    
    /**
     * Get Pinterest share count for a url
     * @param  string $url The URL to get the Pinterest count for
     * @return int  number of +1's
     */
    public static function getPinterestShareCount($url) {
        if (strpos($url, "http") === false):
            $url = 'http://' . $url;
        endif;
        $json = self::curl("http://api.pinterest.com/v1/urls/count.json?callback=p&url=" . $url);
    ...
    /**
     * Get LinkedIn share count for a url
     * @param  string $url The URL to get the LinkedIn count for
     * @return int  number of +1's
     */
    public static function getLinkedInShareCount($url) {
        if (strpos($url, "http") === false):
            $url = 'http://' . $url;
        endif;
        $json = self::curl("http://www.linkedin.com/countserv/count/share?url=" . $url . "&format=json");
        $data = json_decode($json, true);
        if (isset($data['count'])):
            return $data['count'];
        endif;
